...
import { Usuario } from '../../Model/user.database.model.js';
...
export async function Desativar(req, res) {
  ...
  try {
    ...
    res.status(500).json({message: "Erro ao tentar desativar conta. Verificar userService", error: err})
...
export async function Reativar(req, res) {
   try {
      ...
      return res.status(500).json({ message: "Erro so tentar reativar conta, verificar userService" })

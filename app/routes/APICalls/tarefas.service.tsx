...
export default function Tarefas() {
  ...
  const adicionarTarefa = async (
    ...
    try {
      const ApiAdicionarTarefa = await fetch(
        "http://localhost:4000/criar-tarefa",
        {
          method: "POST",
          credentials: "include",
      ...
      if (
        ApiAdicionarTarefa.status === 401 ||
        ApiAdicionarTarefa.status === 403
      ) {
  ...
  const editarTarefa = async (id: any, e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const ApiEditarTarefa = await fetch(
        `http://localhost:4000/editar-tarefa/${id}`,
        {
          method: "PUT",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
      ...
      if (ApiEditarTarefa.ok) {
        setTarefa((prev) =>
          prev.map((tarefa) => (tarefa._id === id ? data.tarefa : tarefa)),
        );
  ...
  const deletarTarefa = async (id: any, e: any) => {
    ...
    try {
      const ApiDeletarTarefa = await fetch(
        `http://localhost:4000/excluir-tarefa/${id}`,
        {
          method: "DELETE",
          credentials: "include",
          headers: { "Content-type": "application/json" },
      ...
      if (ApiDeletarTarefa.ok) {
        setTarefa((prev) => prev.filter((tarefa) => tarefa._id !== id));
